...
export function DeveloperDetailPage() {
  ...
  // Two handles on the same endpoint on purpose: the status toggle and the budget form are
  // separate actions in separate places, and each has to report its own refusal beside itself.
  const updateStatus = useUpdateDeveloper(id);
  const updateBudget = useUpdateDeveloper(id);
  ...
  // Rotate and revoke act on the same row of buttons; only the latest attempt is worth reporting.
  const keyError = revokeKey.error ?? rotateKey.error;
    ...
    <QueryState isPending={developer.isPending} error={developer.error}>
      {developer.data && (
        <div className="stack">
          ...
          {updateStatus.error && <Notice kind="error">{updateStatus.error.message}</Notice>}
          ...
          <div className="grid grid-half">
            <Section title="Budget">
              <div className="stack">
                <FormCard
                  ...
                  action={
                    <button type="submit" className="primary" disabled={updateBudget.isPending}>
                      Save budget
                    </button>
                  }
                  onSubmit={saveBudget}
                ...
                {updateBudget.error && <Notice kind="error">{updateBudget.error.message}</Notice>}
              </div>
            </Section>
            ...
            <Section title="Models">
              <div className="stack">
                ...
                {setModels.error && <Notice kind="error">{setModels.error.message}</Notice>}
              </div>
            </Section>
          </div>
          ...
          <Section title="Keys">
            <div className="stack">
              ...
              {keyError && <Notice kind="error">{keyError.message}</Notice>}
            </div>
          </Section>
        </div>
      )}
